...
## Resumen de cambios
- chatterbox-tts reemplazado por `Mimic3TTS` en `tts.py`, con voz por defecto `es_ES/m-ailabs_low#karen_savage` y re-muestreo a `sample_rate` configurado.
...
- Compartir voz TTS con Lucy: usar el mismo `voice` que en `Proyecto-VSCode` (ej. `es_ES/m-ailabs_low#karen_savage`) para consistencia de timbre.
